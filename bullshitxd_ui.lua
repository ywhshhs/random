...
function library:make_resizable(frame)
	...
	library:connection(uis.InputChanged, function(input, game_event)
		if resizing and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
			local mouse_pos = vec2(input.Position.X, input.Position.Y)
...
function library:window(properties)
	...
	-- mobile menu button (touch devices have no menu keybind): floating toggle
	if uis.TouchEnabled then
		...
		BorderColor3 = Color3.fromRGB(0, 0, 0),
		BorderSizePixel = 0,
		Position = UDim2.new(1, -64, 0.5, -26),
		...
		mbtn.MouseButton1Click:Connect(function()
			local vis = not WINDOW_PATH.Visible
			WINDOW_PATH.Visible = vis
			playerlist.Visible = flags["player_list"] and vis or false
		end)
	end
